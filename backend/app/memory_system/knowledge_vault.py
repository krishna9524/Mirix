...
class EpisodicMemory(BaseMemory):
    ...
    def add_chat_message(self, session_id: str, text: str):
        ...
        try:
            ...
            # 3. Save to Vector DB
            vector_db_service.add_embedding(embedding, new_chunk.id)
            
            # 4. Save the vector index to disk (for persistence)
            vector_db_service.save_index()
    ...
    def retrieve_relevant_chats(self, query_text: str, k: int = 3) -> list[str]:
        ...
        try:
            ...
            chunk_ids = vector_db_service.search_similar(query_text, k)
            
            if not chunk_ids:
                return []

            # 2. Retrieve the text for those chunks from the SQL database
            # We use .in_(chunk_ids) for an efficient SQL query
    ...
    def clear(self):
        # This would be more complex, needing to clear SQL and FAISS
